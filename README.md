...
**Cron:** `RCD_CRON_SECRET` enables `POST /api/cron/notifications` (header `X-RCD-Cron` or JSON `secret`). Each run **re-checks box-league score notifications** for every saved box score in `SEASON_YEARS` (idempotent), then runs handicap logic only during the active handicap calendar window: **match reminders** for unscored matches this week; **standings** when a week completes (rules as in `app.py`). Submitting a handicap score triggers **standings** emails only (not match reminders).
