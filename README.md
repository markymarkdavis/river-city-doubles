...
| **GitHub Actions — `notifications-cron.yml`** | Daily at **15:20 UTC** (~10:20 AM US Eastern in standard time), `POST`s `/api/cron/notifications` (today’s season/week only). Secrets: `NOTIFICATIONS_CRON_URL`, `NOTIFICATIONS_CRON_SECRET` (= `RCD_CRON_SECRET` on Render). |
| **Render Cron — `river-city-doubles-notifications`** | Same schedule (**15:20 UTC** / ~10:20 AM EST), runs `python scripts/run_notification_cron.py` with `RCD_CRON_SECRET` (see `render.yaml`). |
